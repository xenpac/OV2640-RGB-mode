...
int size=2000000;
count=2;
if (s_state->fb_size > size) 
{
	count=1;
}
else
s_state->fb_size=size;

s_state->config.fb_count=count;
    ...
    for(size_t i = 0; i < count; i++)
    {
        ...
        if(!_fb2->buf)
        {
            ESP_LOGI(TAG, "Allocating %d KB frame buffer in SPIRAM", s_state->fb_size/1024);
        ...
        if(!_fb2->buf)
        {
            ESP_LOGE(TAG, "Allocating %d KB frame buffer in SPIRAM Failed", s_state->fb_size/1024);
            free(_fb2);
            goto fail;
        }
...
//	s_state->width=2560; //test
//    s_state->fb_size = s_state->width * s_state->height * 2; //test
	// the dmabuffer length: example 640: 640 * 2 * 4 = 5120 bytes. Thats one DWORD for every Pixel(1280 Pixels)
    size_t line_size = s_state->width * s_state->in_bytes_per_pixel * i2s_bytes_per_sample(s_state->sampling_mode);
    ESP_LOGI(TAG, "Line width: %d Line-DMAsize:%d = width_%d * BpPix_%d * BytesPerSample_%d", s_state->width,line_size,s_state->width,s_state->in_bytes_per_pixel,i2s_bytes_per_sample(s_state->sampling_mode));
    ...
    {
        buf_size /= 2;  // this gets more dmadescriptors than needed!
        dma_per_line *= 2;
    }
    size_t dma_desc_count = dma_per_line * 4; // we use 4 times the needed descriptors, so we can saveley read while dma is in progress
    s_state->dma_buf_width = line_size; // dma buffersize
    s_state->dma_per_line = dma_per_line; 
    s_state->dma_desc_count = dma_desc_count;
    ESP_LOGI(TAG, "MultipleDescr=4 same_DMAbuf size: %d, DMAbufs per line: %d LineDmaSize:%d All_DescrCnt:%d ", buf_size, dma_per_line,line_size,dma_desc_count);
    ESP_LOGI(TAG, "Total DMA buffers Bytes: %d bytes", buf_size * dma_desc_count);
    s_state->dma_buf = (dma_elem_t**) malloc(sizeof(dma_elem_t*) * dma_desc_count); // alloc array of pointers to dma data buffer of type dma_elem_t=4bytes
    if (s_state->dma_buf == NULL)
    {
        return ESP_ERR_NO_MEM;
    }
    s_state->dma_desc = (lldesc_t*) malloc(sizeof(lldesc_t) * dma_desc_count); // alloc array of dma desriptor pointers
    ...
    {
        dma_elem_t* buf = (dma_elem_t*) malloc(buf_size); // alloc dma data buffers
        if (buf == NULL)
        {
            return ESP_ERR_NO_MEM;
        }
        ...
        pd->length = buf_size; // bufsize must be less than 4096 for one descriptor
        if (s_state->sampling_mode == SM_0A0B_0B0C && (i + 1) % dma_per_line == 0)
        {
            pd->length -= 4;
        }
        dma_sample_count += pd->length / 4; // one sample is always 4 bytes. this is the total amount of samples to capture per transaction
        pd->size = pd->length;
        pd->owner = 1;
        pd->sosf = 1;
        pd->buf = (uint8_t*) buf;
...
//	ESP_LOGI(TAG, "Desc[%d]adr:%p size:%u inlen:%u eof:%u owner:%u buf:%p dnext:%p\n",i,pd,pd->size,pd->length,pd->eof,pd->owner,pd->buf,pd->qe.stqe_next);

    }
    s_state->dma_sample_count = dma_sample_count;
//	ESP_LOGI(TAG, "SampleCount per Line:%d Bytes, %d DWORDs",dma_sample_count,dma_sample_count/4);
    return ESP_OK;
}
...
static int i2s_run()
{
    ...
    while(s_state->config.fb_count > 1) // if there is more than one framebuffer
    {
...
// this triggers when one complete line has been received. ie. programmed transfer count is reached.
static void IRAM_ATTR i2s_isr(void* arg)
{
//    Icnt1++;
    ...
    signal_dma_buf_received(&need_yield); // wake dmafilter task, send it the filled dmabuffer address
...
// send the ready frame to the application. called from: dma_finish_frame,
    ...
    if(!s_state->fb->ref) // if buffer is not in use by application
    {
        ...
        else  // not bad, do jpg checks, and send to application
        {
            ...
            if(s_state->fb->len)
            {
                ...
                {
                    uint8_t * dptr = &s_state->fb->buf[s_state->fb->len - 1]; // goto end of data -1
                    while(dptr > s_state->fb->buf)
                    {
                        if(dptr[0] == 0xFF && dptr[1] == 0xD9 && dptr[2] == 0x00 && dptr[3] == 0x00) // test for endmarker jpeg
                        {
                            dptr += 2;
                            s_state->fb->len = dptr - s_state->fb->buf;
                            if((s_state->fb->len & 0x1FF) == 0) // if len is 0x----0000
                            {
                                s_state->fb->len += 1;
                            }
...
// tomk init camera with registers
    ...
    case OV2640_PID:
        *out_camera_model = CAMERA_OV2640;
        ...
        ov2640_init(&s_state->sensor); // init the cam function pointers and the camera
 		ESP_LOGI(TAG, "ov2640 init done");
       break;
...
// init and start i2s engine
   i2s_init();
    ...
    //ToDo: core affinity?
    if (!xTaskCreate(&dma_filter_task, "dma_filter", 4096, NULL, 10, &s_state->dma_filter_task)) // use high priority of 10 (of 25)
    {
