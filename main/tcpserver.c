...
//        if (ret != 1) goto send404;  webpage freezes if 404 is returned, so dont do it
        ...
        if (!strcmp(uri,"/download"))
        {
           if (!IsStreaming)
            {
                ESP_LOGI(TAG,"Download frame.bmp");
            ...
            else
            {
                ...
                sprintf(response,resp_attach_jpg,len);
            }
...
If stream breaks, the webpage streamwindows hangs..still waiting. reload webpage then.
...
//first check for internal commands for the server:
    ...
    {
		 // if streammode is bmp (jpg/bmp) , pixformat must be rgb565  (rgb/yuv)
		 if (value)
		 {
			if (s->pixformat == PIXFORMAT_YUV422) 
    ...
    {
        if (value) // if pixformat is yuv (rgb/yuv), streammode must be jpg (jpg/bmp)
		{
    ...
    else if (!strcmp(variable, "quality")) // adjust jpg quality in conversion routine
    {
	   ...
	   if (Quality > 90) Quality=90; // you get errors  
        ret=1;
    }
...
//check 'variable' for the request parameter
    ...
    {
        sprintf(iobuf,"NetFPS:%d CamFPS:%d I2sFPS:%d CamErrors:%d",NetFPS,HwFPS,I2sFPS,CapErrors);
...
void bmp_mode(int bmp)
{
	...
	ESP_LOGI(TAG,"StreamJPG:%d StreamBMP:%d",Convert,UseBmp);
...
0 = fail, no rgb565 detected, so its yuv or something else! (on the OV2640)
*/
int setbmp(void)
{
